...
local function set_osc_styles()
    ...
    osc_styles = {
        ...
        window_control = "{\\blur1\\bord0.5\\1c&H" .. osc_color_convert(user_opts.window_controls_color) .. "&\\3c&H0&\\fs20\\fn" .. iconfont .. "}",
        ...
        chapter_title = "{\\blur1\\bord0.5\\1c&H" .. osc_color_convert(user_opts.chapter_title_color) .. "&\\3c&H0&\\fs" .. user_opts.chapter_title_font_size .. "\\fn" .. user_opts.font .. "}",
...
local function menu_layout(items)
    ...
    local menu_w = math.min(osc_param.playresx - 40, 760)
    if menu_w < 240 then
...
local function render_menu_window(ass)
    ...
    if mx >= 0 and my >= 0 then
        ...
        if mx >= layout.x0 and mx <= layout.x0 + layout.w and my >= content_top and my <= layout.y0 + layout.h then
    ...
    local header_fs = math.max(16, (user_opts.time_font_size or 16) + 2)
    ass:new_event()
    ...
    ass:an(6)
    ass:append(string.format("{\\bord0\\blur0\\1c&H%s&\\1a&H00&\\fs%d\\fn%s}", fg, math.max(10, header_fs - 6), user_opts.font))
    ass:append(string.format("%d items", #items))
    ...
    for i = 1, visible do
        ...
        if is_sel then
            ...
            ass:draw_start()
            local bg_w = layout.w - pad.l - pad.r - indicator_w - 8 + sp.l + sp.r
        ...
        ass:an(4)
        ass:append(string.format("{\\bord0\\blur0\\1c&H%s&\\1a&H00&\\fs%d\\fn%s\\q2}", text_color, row_fs, user_opts.font))
        local label = item.label or ""
        if #label > 60 then
            local limit = 57
            while limit > 0 and (label:byte(limit) >= 128 and label:byte(limit) < 192) do
                limit = limit - 1
            end
            label = string.sub(label, 1, limit) .. "..."
        end
        ass:append(ass_escape(label))

        if item.current then
            ass:new_event()
            ass:pos(layout.x0 + layout.w - pad.r - menu_sel_padding.r - 4, ry + layout.row_h / 2)
            ass:an(6)
            ass:append(string.format("{\\bord0\\blur0\\1c&H%s&\\1a&H00&\\fs%d\\fn%s}", arrow, row_fs, user_opts.font))
